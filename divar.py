...
def command_line_parser():
    ...
    with open('City Codes/city_codes.json', 'r', encoding='utf-8') as file:
        city_codes = json.load(file)

    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--category', '--cat', '-c',
                        default='apartment-rent',
                        help='Category of posts to search for (default: apartment-rent)')
    parser.add_argument('--city-code', '--city', '--code', '-z',
                        type=int, default=1,
                        help=f'City code or identifier (default: 1; {city_codes.get(1, "Unknown")})')
    parser.add_argument('--result-directory', '--directory', '-d',
                        type=Path, default=Path('Results'),
                        help='Directory in which to store scraping results (default: Results)')
    parser.add_argument('--last-post-date', '--date', '--time', '-t',
                        type=date_parser, default=datetime.now(),
                        help='Date and time for the last post to scrape (default: current date and time)')
    parser.add_argument('--max-pages', '--pages', '-p',
                        type=int, default=4,
                        help='Maximum number of pages to scrape (default: 4)')
    parser.add_argument('--max-retries', '--retries', '-r',
                        type=int, default=5,
                        help='Maximum number of retries for failed downloads (default: 5)')
    parser.add_argument('--show-codes', action='store_true',
                        help='Show all city codes with their names and exit')
...
def main(argv=None):
    ...
    if args.show_codes:
        # Show city codes and exit
        with open('City Codes/city_codes.json', 'r', encoding='utf-8') as file:
            city_codes = json.load(file)
            for code, city_name in city_codes.items():
                print(f"City Code {code}: {city_name}")
    else:
